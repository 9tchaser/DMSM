...
def run_training(*, model_constructor, train_input_fn, total_bs, optimizer, lr, warmup, grad_clip, ema_decay=0.9999, log_dir, exp_name, max_steps=int(1e10)):
  ...
  def model_fn(features, params, mode,labels):
    ...
    assert features['fs_im'].shape[0] == local_bs
    features['us_im'].set_shape([local_bs,2,256,256])
    features['mask'].set_shape([local_bs,1,256,256])
    features['fs_im'].set_shape([local_bs,2,256,256])
    ...
    labels['label'], tf.cast(features['mask'], tf.float32), tf.cast(features['fs_im'], tf.float32)) #### add fs_im as loss
  ...
  estimator = tf.estimator.Estimator(
    ...
    config=tf.estimator.RunConfig(
      ...
      save_checkpoints_secs=1800,  # 30 minutes
...
class EvalWorker:
  def __init__(self, model_constructor, total_bs, dataset):
    ...
    img_batch_shape = [self.local_bs,2,256,256]
  ...
  def _make_sampling_graph(self, img_shape):
    ...
    def _make_inputs(local_bs):
      # Dummy inputs to feed to samplers
      input_x = tf.random.normal([local_bs, *img_shape])
      input_mask_and_us, input_y = self.it.get_next()
      input_mask_and_us['us_im'].set_shape([local_bs,2,256,256])
      input_mask_and_us['mask'].set_shape([local_bs,1,256,256])

      return input_x, input_mask_and_us, input_y

    # Samples
    x,im_us_and_mask,y = _make_inputs(self.local_bs)
    samples_outputs = self.model.samples_fn(dummy_noise=x,y=y['label'], us_im=im_us_and_mask['us_im'], mask=im_us_and_mask['mask'])
    samples_outputs['samples'] = (samples_outputs['samples'] + 1) / 2
    ...
    samples_outputs_complex = {'samples':tf.complex(samples_outputs['samples'][0,0,:,:],samples_outputs['samples'][0,1,:,:])}
    return samples_outputs_complex
  ...
  def _write_eval_and_samples(self, sess, log: utils.SummaryWriter, curr_step, prefix, ema: bool, idx):
    ...
    for k, v in direc:
      np.save(self.logdir + "/test/im_"  + str(idx) + k + ".npy", v)
    log.flush()
  ...
  def run(self, logdir, seed=0, eval_checkpoint=""):
    ...
    with tf.Session(config=config) as sess:
      ...
      trainable_variables = self.model.get_trainables()
      print('num params: {:,}'.format(sum(int(np.prod(p.shape.as_list())) for p in trainable_variables)))
