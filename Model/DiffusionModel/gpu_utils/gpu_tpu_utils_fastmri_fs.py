...
def run_training(
  ...
  if latest_checkpoint:
    print('Latest checkpoint found: {}'.format(latest_checkpoint))
  else:
  ...
  def model_fn(features, params, mode,labels):        
    ...
    features['fs_im'].set_shape([local_bs,2,512,512])
    ...
    labels['label'], tf.cast(features['mask'], tf.float32), tf.cast(features["coil_map"], tf.float32),tf.cast(features["fs_im"], tf.float32))
  ...
  estimator = tf.estimator.Estimator(
    ...
    warm_start_from=latest_checkpoint  # 使用最新的checkpoint进行初始化
  )
