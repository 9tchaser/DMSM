...
class GaussianDiffusion2:
  ...
  def training_losses_fastmri_ssdu(self, denoise_fn, x_start_1, x_start_2,
                                   x_start_3, t, us_im,  coil_map, res_mask,  noise=None):
    """
    ...
    model_output_1 = denoise_fn(x=x_t_1, t=t)
    ...
    partial_kspace_loss = self.partial_kspace_loss_fastmri(us_im = us_im, generated_1= model_output_1, generated_2=model_output_2,
                                           generated_3=model_output_3, res_mask = res_mask, coil_map = coil_map)
  ...
  def partial_kspace_loss_fastmri(self, us_im, generated_1, generated_2,
                                   generated_3, res_mask, coil_map):
      ...
      generated_2 = tf.complex(generated_2[:,0],generated_2[:,1])
      generated_3 = tf.complex(generated_3[:,0],generated_3[:,1])
      ...
      generated_2 = tf.gather_nd(indices=indices, params=generated_2, batch_dims=1) 
      generated_3 = tf.gather_nd(indices=indices, params=generated_3, batch_dims=1) 

      us_im = tf.tile(tf.expand_dims(us_im, axis=1), [1,5,1,1])
      generated_1 = tf.tile(tf.expand_dims(generated_1, axis=1), [1,5,1,1])
      generated_2 = tf.tile(tf.expand_dims(generated_2, axis=1), [1,5,1,1])
      generated_3 = tf.tile(tf.expand_dims(generated_3, axis=1), [1,5,1,1])

      us_im_kspace = self.fft2c(us_im * coil_map)
      generated_1_kspace = self.fft2c(generated_1 * coil_map)
      generated_2_kspace = self.fft2c(generated_2 * coil_map)
      generated_3_kspace = self.fft2c(generated_3 * coil_map)

      # self supervised method
      loss_im_1 = tf.abs(generated_1 - generated_2)
      loss_im_2 = tf.abs(generated_1 - generated_3)
      loss_im_3 = tf.abs(generated_3 - generated_2)
      loss_im = loss_im_1 + loss_im_2 + loss_im_3

      res_mask = tf.cast(res_mask, dtype=tf.complex64)
      loss_kspace_1 = tf.abs(generated_1_kspace- us_im_kspace)
      loss_kspace_2 = tf.abs(generated_2_kspace - us_im_kspace)
      loss_kspace_3 = tf.abs(generated_3_kspace - us_im_kspace)
      loss = loss_kspace_1 + loss_kspace_2 +  2* loss_kspace_3
